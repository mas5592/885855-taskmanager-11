...
const createColorsMarkup = (colors, currentColor) => {
  return colors.slice().map((color, index) => {
    const checked = currentColor === color ? `checked` : ``;

    return (
      `<input type="radio" id="color-${color}-${index}" class="card__color-input card__color-input--${color} visually-hidden" name="color" value="${color}" ${checked}>
...
const createRepeatingDaysMarkup = (days, repeatingDays) => {
  return days.slice().map((day, index) => {
